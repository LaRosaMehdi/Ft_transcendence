...
class CheckUserInactivityCron(CronJobBase):
    ...
    def do(self):
        print('Checking user inactivity and updating statuses...')
            ...
            try:
                if user.username != 'root':
                    if now - user.last_activity > inactivity_limit and user.status != 'offline':
                        logger.info(f"User {user.id} is inactive. Updating status to offline.")
                        user.status = 'offline'
...
class CheckUserUnverified(CronJobBase):
    RUN_EVERY_MINS = 0.1
    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'aouth.cron.CheckUserUnverified'  
            ...
            try:
                if user.username != 'root':
                    if user.is_verified is False and now - user.validation_code_expiration > unverified_limit:
                        logger.info(f"User {user.id} is unverified. Deleting user.")
                        user.delete()
                        user.save()
            except Exception as e:
